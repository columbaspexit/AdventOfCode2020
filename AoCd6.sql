...
-- Again, with the copy/pasting in from the input file...
set @input = 'clvxybjp
kripatlzy
...
create table #Persons(pAnswer varchar(26),gID int,pID int identity)
-- 
...
select *
into #GrpxLtr
from #Groups cross join #Letters
...
;with cte as(
    select pID
        -- Use the ''s to id which group a person is in...
        ,count(case when pAnswer = '' then pAnswer end) over(order by pID rows unbounded preceding) + 1[groupID]
    from #Persons
)
update #Persons
    set gID = groupID
...
-- Each person gets an entry for each letter, if that letter is in their answer
...
, cte2 as(
    select tl.gID
        ,np.NumPersons
        ,tl.letter
        -- Number of yeses for a given letter, among group members...
        ,sum(tl.n)[nOccr]
    from #PrsnxLtr tl
    ...
    group by tl.gID
        ,np.NumPersons
        ,tl.letter
    -- limit entries to the letters with yeses among all group members
